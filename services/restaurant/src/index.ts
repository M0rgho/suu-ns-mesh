...
const DELIVERY_SERVICE_URL =
  process.env.DELIVERY_SERVICE_URL || "http://localhost:3003";
...
app.post("/api/restaurant/prepare-order", (req: Request, res: Response) => {
  ...
  items.forEach((item) => {
    if (!inventory[item.name] || inventory[item.name] < item.quantity) {
      return res
        .status(400)
        .json({ error: `Not enough ${item.name} in inventory` });
    }
  });
  ...
  setTimeout(async () => {
    ...
    try {
      await axios.post(`${DELIVERY_SERVICE_URL}/api/delivery/dispatch`, {
        orderId,
      });
